...
def delete_workgroup(workgroup_name):
    try:
        ...
        if(response_wg_delete['ResponseMetadata']['HTTPStatusCode'] != 200):
            raise Exception("Status code from deletion of Athena Workgroup"+workgroup_name+" is not 200")
        else:
            logging.info("Athena Workgroup "+workgroup_name+" is deleted successfully")     
...
def delete_database(bucket_name,athena_bucket):
    try:
        ...
        if(response_delete_db['ResponseMetadata']['HTTPStatusCode'] != 200):
            raise Exception("Status code from deletion of Athena Table "+table+" is not 200")
        else:
        ...
        if(response_delete_db['ResponseMetadata']['HTTPStatusCode'] != 200):
            raise Exception("Status code from deletion of Athena Database "+database+" is not 200")
        else:
        ...
        logging.info("Delete logs will be published to Athena logging Bucket, therefore deleting immediately Bucket cause error. Sleep 5 Mins")
        time.sleep(300)
        ...
        try:
            ...
            if(response['ResponseMetadata']['HTTPStatusCode']==200):
                ...
                for buckets in response['Buckets']:
                    if client.get_bucket_location(Bucket=buckets['Name'])['LocationConstraint'] == region:
                ...
                raise Exception("Response Code is not 200. Therefore abondoning the deletion process ")
